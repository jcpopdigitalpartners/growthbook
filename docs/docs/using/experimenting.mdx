...
percentage, as well as customize the split per variation. You can also target an experiment at specific
attribute values.
...
Quite often there is a need to de-risk a new A/B test by running the control at a higher percentage of
...
trying to improve or measure the impact of the change from your experiment.
Secondary metrics are used to learn about experiment impacts, but are not primary objectives. Guardrail metrics are your “don’t break this” metrics. They help you confirm that a winning experiment is not quietly damaging something important.

<MaxWidthImage maxWidth={600}>
  ![GrowthBook Metric Selector](/images/using/metrics-modal.png)
</MaxWidthImage>

It is best to choose experiment metrics that are as close as possible to the treatment— and, ideally, measure the event itself. For
...
need to your experiment, but we suggest each experiment have only a few primary metrics that are used for
...
trying to improve. These metrics are often referred to as the OEC for Overall Evaluation Criterion. It is
...
the data exists in your data warehouse. This lets you reprocess past experiments after adding or redefining metrics.
